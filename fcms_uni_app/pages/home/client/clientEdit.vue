<template>
  <view class="bg-white nys_content">
    ...
    >
      ...
      <view class="form-block" style="border-color: #F56C6C;">
    ...
    <view class="footer-view">
      <u-button :plain="true" type="error" @click="submit">提 交</u-button>
    </view>
    <view class="footer-view">
      <u-button type="error" @click="actionShow = true">删 除</u-button>
    </view>

    <u-action-sheet :actions="actionList" @select="selectClick" @close="actionShow = false" title="确定删除吗？" description="谨慎操作，删除后该客户信息无法恢复。" cancelText="取消" :show="actionShow"></u-action-sheet>
  </view>
</template>
  ...
  methods: {
    ...
    submit() {
      this.$refs.uForm.validate().then(res => {
        this.$service.put(this.$api.addClient, this.form).then(res => {
          uni.showToast({
            title: '修改成功',
            icon: 'success'
          })
    ...
    selectClick(obj) {
      if (obj.id == 0) {
        this.$service.delete(this.$api.getClient+this.form.clientId).then(res => {
